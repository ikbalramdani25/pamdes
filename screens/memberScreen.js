...
import caterScreen from './caterScreen';
function memberScreen({navigation}){
    var card =[];
    for (let i = 0; i < 10; i++) {
      card.push(
        <Pressable style={styles.memberBox} onPress={()=> navigation.navigate('Input Data')} >
            <View style={styles.nameBox} >
                <Text style={{fontWeight:"bold"}}>UDIN</Text>
                <Text>01-001</Text>
                <Text>Jetak</Text>
            </View>
            <Pressable onPress={()=> navigation.navigate('Input Data')} style={styles.btnNext}><Text style={{color:"white",fontSize:18, textAlign:"center"}}>{">"}</Text></Pressable>
          </Pressable>
      );
